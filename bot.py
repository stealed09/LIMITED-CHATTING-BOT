"""
bot.py - Main entry point for the Livegram-style advanced support bot.

Setup:
1. pip install pyTelegramBotAPI
2. Edit config.json: set bot_token and admin_id
3. python bot.py

Or set BOT_TOKEN and ADMIN_ID as environment variables (for Railway/VPS).
"""
...
def setup_config() -> dict:
    """
    Load config, applying environment variables if set.
    Environment variables override config.json.
    """
    config = load_config()

    # Allow overriding via env vars (useful for Railway, Heroku, etc.)
...
def main():
    ...
    # Order matters: commands first, then admin replies, then user messages
    ...
    # Polling with auto-restart on error
    while True:
        try:
